...
public interface JobDao {
	...
	/*@Select("SELECT j . * , c.currency_name ,t.sections FROM jobs j, currency c, templates t WHERE j.currency_value=c.currency_value and j.template_id=t.id and j.created_byId=#{id} or j.hiring_user_id=#{id} or j.template_id=#{id}")*/
	@Select("SELECT j . * , c.currency_name ,t.sections as sectionName FROM jobs j, currency c, templates t WHERE j.template_id=t.id  and j.currency_value=c.currency_value and j.created_byId=#{id} or j.hiring_user_id=#{id} and j.template_id=#{id}")
	...
	@Insert("Insert into jobs (template_id,sectionName,diversity,title,description,location,created_at,created_byId,hiring_user_id,skills,status,job_type,compensation,payrate_type,currency_value,minimum_experience,maximum_experience, duration,showCompensation,start_date,preferredCheck,diverseCheck,department,quota,diverse,others,diverseType) values (#{templateId},#{sectionName},#{diversityList},#{title},#{description},#{location},NOW(),#{createdById},#{hiringUserId},#{skills},#{status},#{jobType},#{compensation},#{payrateType},#{currency},#{minimumExperience},#{maximumExperience},#{duration},#{showCompensation},#{startDate},#{preferredCheck},#{diverseCheck},#{department},#{quota},#{diverse},#{others},#{diverseType})")
